...
Deno.serve(async (req) => {
  try {
    ...
    const placeholders = {
      ...
      vervaldatum: document.expiry_date || '—',
    };

    // Check for custom template
    let emailSubject, emailBody;
    const templates = await base44.asServiceRole.entities.EmailTemplate.filter({
      template_key: 'nieuw_document',
      is_active: true,
    });
    ...
    if (templates.length > 0) {
      ...
      emailSubject = `Nieuw HR-document beschikbaar: ${document.name}`;
      emailBody = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: white; padding: 24px; border: 1px solid #e2e8f0; border-top: none;">
            ...
            <p style="color: #475569; line-height: 1.6;">Er is een nieuw HR-document voor je beschikbaar gesteld in het Interdistri portaal.</p>
            <div style="background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <table style="width: 100%; font-size: 14px; color: #1e293b;">
                <tr><td style="padding: 4px 0; color: #64748b; width: 120px;">Document:</td><td style="padding: 4px 0; font-weight: 600;">${document.name}</td></tr>
                <tr><td style="padding: 4px 0; color: #64748b;">Type:</td><td style="padding: 4px 0;">${document.document_type || '-'}</td></tr>
                ${document.expiry_date ? `<tr><td style="padding: 4px 0; color: #64748b;">Vervaldatum:</td><td style="padding: 4px 0;">${document.expiry_date}</td></tr>` : ''}
                ${document.notes ? `<tr><td style="padding: 4px 0; color: #64748b;">Opmerking:</td><td style="padding: 4px 0;">${document.notes}</td></tr>` : ''}
              </table>
            </div>
            <p style="color: #475569; font-size: 14px; line-height: 1.6;">Je kunt dit document bekijken via het Interdistri portaal.</p>
          </div>
