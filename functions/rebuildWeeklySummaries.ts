...
async function recalcWeek(svc, year, week_number) {
  ...
  customers.forEach(c => {
    ...
    const hourArt = custArticles.find(a => (a.description || '').toLowerCase().match(/uur|hour/));
    const kmArt = custArticles.find(a => (a.description || '').toLowerCase().match(/km|kilometer/));
    const stopArt = custArticles.find(a => (a.description || '').toLowerCase().includes('stop'));
    const stukArt = custArticles.find(a => (a.description || '').toLowerCase().match(/stuk|pakket/));
    rateMap[c.id] = {
  ...
  // Trip/SPW lookups
  const tripsByTE = {};
  trips.forEach(t => { if (t.time_entry_id) { (tripsByTE[t.time_entry_id] = tripsByTE[t.time_entry_id] || []).push(t); } });
  const spwsByTE = {};
  spws.forEach(s => { if (s.time_entry_id) { (spwsByTE[s.time_entry_id] = spwsByTE[s.time_entry_id] || []).push(s); } });
  ...
  for (const [custId, data] of Object.entries(customerMap)) {
    ...
    const payload = {
      customer_id: custId, customer_name: custNameMap[custId] || 'Onbekend',
      year, week_number,
      total_hours: r2(data.hours), total_km: r2(data.km),
      hour_revenue: r2(data.hourRevenue), km_revenue: r2(data.kmRevenue),
    ...
    if (existing) { await svc.entities.WeeklyCustomerSummary.update(existing.id, payload); custUpdated++; }
    else { await svc.entities.WeeklyCustomerSummary.create(payload); custCreated++; }
  }
  ...
  for (const [empId, data] of Object.entries(employeeMap)) {
    ...
    const payload = {
      employee_id: empId, employee_name: empNameMap[empId] || 'Onbekend',
      year, week_number,
    ...
    if (existing) { await svc.entities.WeeklyEmployeeSummary.update(existing.id, payload); empUpdated++; }
    else { await svc.entities.WeeklyEmployeeSummary.create(payload); empCreated++; }
  }

  return { custCreated, custUpdated, custDeleted, empCreated, empUpdated, empDeleted };
}
  ...
  try {
    ...
    for (let year = start_year; year <= end_year; year++) {
      ...
      for (let week = 1; week <= maxWeek; week++) {
        try {
          const res = await recalcWeek(svc, year, week);
          totalWeeks++;
