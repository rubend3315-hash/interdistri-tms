...
Deno.serve(async (req) => {
  try {
    ...
    // Helper to embed signature image - use Jimp to flatten transparency and convert to JPEG
    const addSignatureImage = async (url, x, currentY) => {
      try {
        ...
        // Create white background image same size and composite signature onto it
        const bg = new Jimp({ width: w, height: h, color: 0xFFFFFFFF });
        bg.composite(image, 0, 0);
        
        // Export as JPEG buffer (no transparency issues)
