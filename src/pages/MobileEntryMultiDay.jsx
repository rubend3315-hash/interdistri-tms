...
import { format, getWeek, getYear, addDays } from "date-fns";
...
export default function MobileEntryMultiDay() {
  ...
  const [expenseData, setExpenseData] = useState({
    date: format(new Date(), 'yyyy-MM-dd'),
    category: "Brandstof", description: "", amount: "",
    receipt_file: null, receipt_url: ""
  });
  ...
  const calculateHours = (start, end, breakMinutes, startDate, endDate) => {
    ...
    // If end date is after start date, add 24h per day difference
      ...
      <div className={`fixed top-0 right-0 h-full w-72 bg-white z-50 transform transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-slate-200">
            <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 rounded-xl text-red-600 hover:bg-red-50 transition-colors">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-blue-100">Meerdaagse App</p>
            </div>
          </div>
          <button onClick={() => setMenuOpen(true)} className="p-1.5 hover:bg-white/20 rounded-lg">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="home">
            <div className="bg-white rounded-lg p-2.5 text-slate-900 mb-2 border-2 border-blue-200">
              ...
              {welcomeMessage ? (
                <p className="text-sm text-slate-600 whitespace-pre-line">{welcomeMessage.message}</p>
              ) : (
                <p className="text-sm text-slate-600">
                  Welkom op je werkdag, {currentEmployee ? `${currentEmployee.first_name} ${currentEmployee.last_name}` : user?.full_name}!
                  Werk met plezier en aandacht, en rijd veilig. Samen zorgen we dat alles op tijd op de juiste plek komt. Succes vandaag! 🚛
                </p>
              )}
            </div>
                      ...
                      return (
                        <div key={day} className={`p-3 rounded-lg border ${isToday ? 'border-blue-300 bg-blue-50' : isWeekend ? 'border-slate-200 bg-slate-50' : 'border-slate-200 bg-white'}`}>
                          <div className="flex items-start justify-between">
                            ...
                            <div className="text-right">
                              <Badge className={`text-xs ${shiftValue === '-' ? 'bg-slate-100 text-slate-600 border-slate-200' : shiftValue === 'Vrij' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>
                                {shiftValue}
                              </Badge>
          ...
          <TabsContent value="ritten" className="space-y-4">
            <Card className="bg-blue-900 text-white">
              ...
              <CardContent>
                <p className="text-sm text-blue-100">Ritten (Verblijfskosten art. 40 CAO)</p>
              </CardContent>
            </Card>
              ...
              <Card key={index} className="border-2 border-slate-200">
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select value={trip.vehicle_id} onValueChange={(v) => {
                      const newTrips = [...trips];
                  ...
                  <div className="space-y-1">
                    ...
                    <Select value={trip.damage_occurred || "Nee"} onValueChange={(v) => {
                      const newTrips = [...trips];
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input type="number" value={trip.start_km} onChange={(e) => {
                        const newTrips = [...trips];
                    ...
                    <div className="space-y-1">
                      ...
                      <Input type="number" value={trip.end_km} onChange={(e) => {
                        const newTrips = [...trips];
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input type="number" step="0.01" value={trip.fuel_liters || ""} onChange={(e) => {
                        const newTrips = [...trips];
                    ...
                    <div className="space-y-1">
                      ...
                      <Input type="number" step="0.01" value={trip.adblue_liters || ""} onChange={(e) => {
                        const newTrips = [...trips];
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input type="number" value={trip.fuel_km || ""} onChange={(e) => {
                        const newTrips = [...trips];
                    ...
                    <div className="space-y-1">
                      ...
                      <Input type="number" step="0.01" value={trip.charging_kwh || ""} onChange={(e) => {
                        const newTrips = [...trips];
                  ...
                  <div className="pt-3 border-t">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        ...
                        <Select value={trip.customer_id || "none"} onValueChange={(v) => {
                          const newTrips = [...trips];
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <Input type="number" value={trip.planned_stops} onChange={(e) => {
                            const newTrips = [...trips];
                      ...
                      <div className="space-y-1">
                        <Label className="text-xs">Opmerkingen</Label>
                        <Textarea value={trip.notes} onChange={(e) => {
                          const newTrips = [...trips];
                          newTrips[index] = { ...trip, notes: e.target.value };
                          setTrips(newTrips);
                        }} rows={2} placeholder="Bijzonderheden, vertragingen, problemen..." />
                      </div>
                    </div>
                  </div>
                </CardContent>
            ...
            <Button
              ...
              onClick={() => setTrips([...trips, {
                ...
                vehicle_id: "", damage_occurred: "Nee", start_km: "", end_km: "",
                fuel_liters: "", adblue_liters: "", fuel_km: "", charging_kwh: "",
                customer_id: "", route_name: "", planned_stops: "", notes: ""
              }])}
            >
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      {inspectionData.damage_photos.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs text-emerald-600 font-medium">✓ {inspectionData.damage_photos.length} foto('s) toegevoegd</p>
                          ...
                          <div>
                            <p className="font-medium text-slate-900">
                              ...
                              {entry.end_date && entry.end_date !== entry.date && (
                                <span className="text-slate-500"> → {format(new Date(entry.end_date), "d MMM", { locale: nl })}</span>
                              )}
                            </p>
          ...
          <TabsContent value="links" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-2">
                <a href="https://www.mijn.bumper.nl" target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100">
                  <div className="flex items-center gap-2">
                ...
                <a href="https://werknemer.loket.nl" target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100">
                  <div className="flex items-center gap-2">
      ...
      <Dialog open={showSignatureDialog} onOpenChange={setShowSignatureDialog}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500 text-center">Teken je handtekening hierboven</p>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={clearSignature}>Wissen</Button>
              <Button className="flex-1 bg-blue-600 hover:bg-blue-700" onClick={saveSignature}>Opslaan</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
