...
export function useMobileForm({ isMultiDay = false, currentEmployee, businessMode = "HANDMATIG" }) {
  ...
  const generateAutoRit = useCallback((startTime, endTime, postNLCustomerId) => {
    ...
    if (durationMin <= 10) return; // Need at least 10 min for 5+5 margin

    // start + 5 min, end - 5 min (5 min inner margin)
    const startMin = sH * 60 + sM + 5;
    const endMin = ((eH * 60 + eM) - 5 + 1440) % 1440;
    const fmt = (m) => {
