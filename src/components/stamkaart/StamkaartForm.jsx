...
export function StamkaartRow({ label, required, compact, children }) {
  const w = compact ? 150 : 240;
  return (
    <div className="flex items-center" style={{ minHeight: 30 }}>
      <span style={{ fontSize: 13, lineHeight: 1.3, width: w, minWidth: w, paddingRight: 8, textAlign: 'right' }} className="text-slate-600 truncate">{label}{required && " *"}</span>
      <div className="flex-1 min-w-0 min-h-[26px] flex items-center">{children}</div>
    </div>
  );
}
...
export default function StamkaartForm({ 
  ...
  return (
    <div className="mx-auto" style={{ maxWidth: 720, lineHeight: 1.35 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <StamkaartRow label="Loonheffingskorting?">
          <RadioGroup
            value={lhToepassen}
        ...
        <StamkaartRow label="Bijzonderheden">
          <textarea className="text-xs px-2 py-1.5 bg-white border border-slate-400/60 shadow-none rounded-md min-h-[80px] resize-none w-full focus:outline-none focus:ring-1 focus:ring-ring" value={data.financiele_situatie || ""} onChange={e => update("financiele_situatie", e.target.value)} placeholder="Eventueel..." />
        </StamkaartRow>
      </div>
