  ...
  const saveDataMutation = useMutation({
    ...
    onSuccess: async (createdRecords) => {
      ...
      // Auto-trigger conversion to RapportageRit
       try {
         // Convert import records to RapportageRit
         const convertedRits = createdRecords.map(record => {
           const datum = record.data?.['Datum'] || '';
           const dateStr = datum; // Format: DD-MM-YYYY or similar
         ...
         await base44.asServiceRole.entities.RapportageRit.bulkCreate(convertedRits);
         toast.success(`Data opgeslagen en ${convertedRits.length} ritten geconverteerd`);
